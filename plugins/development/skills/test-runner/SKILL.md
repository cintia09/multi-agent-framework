...
```
runner.sh --target-dir <dir> [--config <path>] [--json]
```
...
`--config <path>` is **sourced as a shell snippet**; `TEST_CMD` is
then executed via `eval` inside `cd <target>`. This is intentional —
the memory-first pattern needs full shell flexibility to wrap arbitrary
test runners (pytest selectors, custom wrappers, env-var preludes).
Consequences:
...
* Never source a `--config` file fetched over the network without
  human review first.
* The skill makes **no attempt to sandbox** the test runner — anything
  it can do, the calling shell can do.
...
2. **`--config <path>`** sourced as a shell snippet. The calling role
   ...
   A config file MUST set `TEST_CMD` and MAY set `TEST_LABEL` and
   `PASS_CRITERION` (`exit0` or `regex:<pattern>`). Example for a
   device-attached test box:

   ```sh
   # .codenook/memory/knowledge/test-runner-config-rfsw-hub-dut/index.md
   # (sourced as $CONFIG by runner.sh after extraction)
   TEST_LABEL="ssh-dut"
   TEST_CMD="ssh hub-lab@10.0.31.42 'cd /opt/aphg && pytest -q tests/'"
   PASS_CRITERION="regex:^=+\\s+\\d+\\s+passed"
   ```
