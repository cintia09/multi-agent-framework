#!/usr/bin/env bash
set -euo pipefail
# When a goal is verified: update progress tracking
INPUT=$(cat)
TASK_ID=$(echo "$INPUT" | jq -r '.task_id // ""')
CWD=$(echo "$INPUT" | jq -r '.cwd // "."')
AGENTS_DIR="$CWD/.agents"

[ -z "$TASK_ID" ] || [ "$TASK_ID" = "null" ] && { echo '{"status": "ok"}'; exit 0; }

sql_escape() { echo "$1" | sed "s/'/''/g"; }

if [ -f "$AGENTS_DIR/events.db" ]; then
  TASK_ID_ESC=$(sql_escape "$TASK_ID")
  sqlite3 "$AGENTS_DIR/events.db" "INSERT INTO events(timestamp,event_type,task_id,detail) VALUES(strftime('%s','now'),'goal_verified','$TASK_ID_ESC','Goal verified');" 2>/dev/null || true
fi

echo '{"status": "ok"}'
