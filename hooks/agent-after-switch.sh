...
if [ -f "$AGENTS_DIR/events.db" ]; then
  ...
  if ! sqlite3 "$AGENTS_DIR/events.db" "INSERT INTO events(timestamp,event_type,agent,detail) VALUES(strftime('%s','now'),'agent_switch','$AGENT_ESC','Switched to $AGENT_ESC');" 2>/dev/null; then
