...
- **期望**: 输出严格包含 spec §10 列出的全部 18 个公共函数（`init_memory_skeleton`, `scan_memory`, `scan_knowledge`, `read_knowledge`, `write_knowledge`, `patch_knowledge`, `replace_knowledge`, `promote_knowledge`, `archive_knowledge`, `scan_skills`, `read_skill`, `write_skill`, `patch_skill`, `read_config_entries`, `upsert_config_entry`, `match_entries_for_task`, `find_similar`, `has_hash`, `append_audit`），少一个即失败
...
- **期望**: 主调用 wall 时间 ≤ 200ms；返回 JSON 含 `enqueued_jobs`；`pgrep -f knowledge_extractor` 仍存活
