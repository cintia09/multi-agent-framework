...
def test_hitl_prepare_envelope(installed_workspace: Path) -> None:
    """hitl prepare emits a JSON envelope with the expected keys."""
    ...
    (queue_dir / f"{eid}.json").write_text(
        _json.dumps(entry), encoding="utf-8"
    )

    cp = _run(
        _bin_cmd(ws) + ["hitl", "prepare", "--id", eid],
        check=False,
    )
    assert cp.returncode == 0, f"stdout={cp.stdout!r} stderr={cp.stderr!r}"
    envelope = _json.loads(cp.stdout)
    assert envelope.get("eid") == eid
    assert "html_out" in envelope
    assert "ansi_out" in envelope
    assert "prompt_template" in envelope
