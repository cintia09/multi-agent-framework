...
当任务 `workflow_mode: "3phase"` 时，Reviewer 负责以下步骤:

| Phase | 步骤 | 职责 |
|-------|------|------|
| 1 | `design_review` | 审查设计文档/ADR/DFMEA，批准或退回 Designer |
| 2 | `code_reviewing` | Track C — 持续审查 Implementer 的代码提交 |

与 Simple 模式的区别:
- Phase 1 增加正式设计审查门禁，必须通过才能进入实现
- Phase 2 中 Reviewer 作为 Track C 与 Implementer 并行，实时反馈
- 代码审查退回 (`code_reviewing→implementing`) 触发修复循环
